	...
	<view>
		<view class="order">
			<view class="row" @click="onDetail(item.order_no)" v-for="(item,index) in orderList" :key="item._id">
				...
				<view class="btnGroup">
					<button :disabled="item.complete" type="primary" size="mini" @click.stop="onComplete(item._id)">完成</button>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			// 获取所有用户的订单
			async getOrderList(){
				let res = await orderCloudObj.getAll()
			...
			// 完成订单
			async onComplete(id){
				await orderCloudObj.completeOrder(id)
				this.getOrderList()
			}
		}
	}
